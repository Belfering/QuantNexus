...
function App() {
  ...
  // Check for crash recovery data on mount and automatically move to trash
  useEffect(() => {
    if (!userId) return

    const autoDiscardUnsavedWork = async () => {
      try {
        ...
        if (storedData) {
          ...
          // Only process recovery data if it's less than 24 hours old
          ...
          if (recoveryAge < MAX_RECOVERY_AGE && parsed.bots && parsed.bots.length > 0) {
            ...
            console.log('[Recovery] Automatically moving to trash...')
            ...
            console.log('[Recovery] Automatically moved', parsed.bots.length, 'bot(s) to trash')
          }
        ...
        console.error('[Recovery] Failed to auto-discard unsaved work:', err)
      }
    }

    autoDiscardUnsavedWork()
  }, [userId, backtestMode, backtestCostBps])
  ...
  useEffect(() => {
    ...
    for (const bot of savedBots) {
      ...
      try {
        ...
        console.error(`[TickerContrib] Error processing bot ${bot.name}:`, err)
      }
    }
  }, [savedBots, analyzeBacktests, analyzeTickerContrib, runAnalyzeTickerContribution, callChainsById, backtestMode])
        ...
        {tab === 'Dashboard' ? (
          ...
          <Suspense fallback={<div className="p-4 text-muted">Loading Analyze...</div>}>
            <AnalyzeTab
              ...
              dashboardCash={dashboardCash}
              dashboardInvestmentsWithPnl={dashboardInvestmentsWithPnl}
              positionLedger={[]} // TODO: Connect to useAlpacaPortfolio when live/paper trading is enabled in Analyze tab
              // Action callbacks
              handleNexusBuy={handleNexusBuy}
