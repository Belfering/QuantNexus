...
export function useBacktestRunner({ callChainsById: _callChainsById, customIndicators = [] }: UseBacktestRunnerOptions) {
  ...
  const backtestEngine = useBacktestStore((s) => s.backtestEngine)
  ...
  const runBacktestForNode = useCallback(
    async (node: FlowNode): Promise<BacktestRunResult> => {
      ...
      console.log(`[Backtest] Calling server API with engine=${backtestEngine}, mode=${backtestMode}, costBps=${backtestCostBps}, customIndicators=${customIndicators.length}...`)
      const response = await fetch(`${API_BASE}/backtest`, {
        ...
        body: JSON.stringify({
          ...
          customIndicators,
          engine: backtestEngine, // 'node' or 'rust'
        }),
      })
      ...
      // Debug: log the raw metrics to help diagnose field name issues
      console.log('[Backtest] Raw metrics from server:', serverResult.metrics)
      ...
      const metrics = {
        ...
        // Rust server uses 'vol', Node.js server uses 'volatility'
        vol: serverResult.metrics?.vol ?? serverResult.metrics?.volatility ?? 0,
        maxDrawdown: serverResult.metrics?.maxDrawdown ?? serverResult.metrics?.max_drawdown ?? 0,
        // Rust server uses short names, Node.js uses *Ratio suffix
        calmar: serverResult.metrics?.calmar ?? serverResult.metrics?.calmarRatio ?? 0,
        sharpe: serverResult.metrics?.sharpe ?? serverResult.metrics?.sharpeRatio ?? 0,
        sortino: serverResult.metrics?.sortino ?? serverResult.metrics?.sortinoRatio ?? 0,
        treynor: serverResult.metrics?.treynor ?? serverResult.metrics?.treynorRatio ?? 0,
        beta: serverResult.metrics?.beta ?? 0,
        winRate: serverResult.metrics?.winRate ?? serverResult.metrics?.win_rate ?? 0,
        bestDay: serverResult.metrics?.bestDay ?? serverResult.metrics?.best_day ?? 0,
        worstDay: serverResult.metrics?.worstDay ?? serverResult.metrics?.worst_day ?? 0,
        avgTurnover: serverResult.metrics?.avgTurnover ?? serverResult.metrics?.avg_turnover ?? 0,
        avgHoldings: serverResult.metrics?.avgHoldings ?? serverResult.metrics?.avg_holdings ?? 0,
      }
    ...
    [backtestMode, backtestCostBps, backtestEngine, customIndicators]
  )
