...
export function useBotOperations({
  ...
  const handleImport = useCallback(() => {
    console.log('[Import] handleImport called, activeBot:', activeBot?.id)
    if (!activeBot) {
      console.warn('[Import] No active bot - cannot import')
      alert('Please select or create a model first before importing.')
      return
    }
    ...
    input.onchange = async () => {
      ...
      try {
        ...
        const MAX_OTHER_SIZE = 5 * 1024 * 1024 // Increased from 1.5MB for large stress tests
        const maxSize = format === 'composer' ? MAX_COMPOSER_SIZE : MAX_OTHER_SIZE
        const maxSizeLabel = format === 'composer' ? '20MB' : '5MB'
