...
app.post('/api/backtest', async (req, res) => {
  try {
    const { payload, mode = 'CC', costBps = 5, customIndicators = [], engine = 'node' } = req.body
    ...
    // Route to Rust vectorized engine if requested
    if (engine === 'rust') {
      const RUST_SERVER = process.env.RUST_SERVER || 'http://localhost:3030'
      console.log(`[Backtest] Routing to Rust engine at ${RUST_SERVER} with mode=${mode}, costBps=${costBps}...`)
      const startTime = Date.now()

      try {
        const rustResponse = await fetch(`${RUST_SERVER}/api/backtest`, {
          method: 'POST',
        ...
        if (!rustResponse.ok) {
          const errText = await rustResponse.text()
          throw new Error(`Rust server error: ${rustResponse.status} - ${errText}`)
        }

        const rustResult = await rustResponse.json()
        const elapsed = Date.now() - startTime
        console.log(`[Backtest] Rust engine completed in ${elapsed}ms - CAGR: ${((rustResult.metrics?.cagr || 0) * 100).toFixed(2)}%`)

        // Return Rust result directly (already in compatible format)
        return res.json({
          success: true,
          metrics: rustResult.metrics,
          equityCurve: rustResult.equity_curve,
          benchmarkCurve: rustResult.benchmark_curve,
          allocations: rustResult.allocations,
          engine: 'rust',
        })
      } catch (rustErr) {
        console.error(`[Backtest] Rust engine error: ${rustErr.message}`)
        return res.status(500).json({ error: `Rust engine: ${rustErr.message}` })
      }
    }

    // Default: Use Node.js backtest engine
    const useRustIndicators = engine !== 'js'
    console.log(`[Backtest] Running Node.js backtest for unsaved strategy with mode=${mode}, costBps=${costBps}, customIndicators=${customIndicators.length}, rustIndicators=${useRustIndicators}...`)
    const startTime = Date.now()

    const result = await runBacktest(payloadStr, { mode, costBps, customIndicators, useRustIndicators })
    ...
    res.json({
      ...
      allocations: result.allocations,
      compression: result.compression,
      engine: useRustIndicators ? 'node' : 'js',
    })
  } catch (e) {
    console.error('[Backtest] Error:', e)
    res.status(500).json({ error: String(e?.message || e) })
