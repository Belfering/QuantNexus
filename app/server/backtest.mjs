...
// Full ticker cache - stores ALL loaded tickers with full history
// Memory usage: ~12k tickers × 8k bars × 50 bytes = ~4.8GB (requires adequate RAM)
...
// Memory limit - LRU cache for most-used tickers
// Note: All 12k tickers with full history requires ~10GB RAM
// To cache all: set to 15000 and run with --max-old-space-size=16384
...
export async function runBacktest(payload, options = {}) {
  ...
  const useRustIndicators = options.useRustIndicators !== false // Default true, set false for pure JS
  ...
  // Skip if useRustIndicators is false (pure JS mode)
  if (useRustIndicators) {
    ...
    console.log(`[Backtest] Using pure JavaScript indicators (Rust disabled)`)
  }
